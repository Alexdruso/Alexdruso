...
USER_NAME = os.environ.get('USER_NAME', 'Alexdruso')
...
def recursive_loc(owner, repo_name, data, cache_comment, addition_total=0, deletion_total=0, my_commits=0, cursor=None):
    ...
    request = requests.post('https://api.github.com/graphql', json={'query': query, 'variables': variables}, headers=HEADERS) # cannot use simple_request(), because we want to save the file before raising Exception
    if request.status_code == 200:
...
if __name__ == '__main__':
    """
    Adapted from Andrew Grant (Andrew6rant): https://github.com/Andrew6rant/Andrew6rant
    """
    print('Calculation times:')
    user_data, user_time = perf_counter(user_getter, USER_NAME)
    OWNER_ID, acc_date = user_data
    formatter('account data', user_time)
    # 'Uptime' counts from the GitHub account creation date; replace with a
    # datetime.datetime(YYYY, M, D) birthday to display age instead
